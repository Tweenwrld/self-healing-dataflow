...
class Config: 
    BASE_DIR = os.getenv('PIPELINE_BASE_DIR', '/home/cliente/Desktop/self-healing-dataflow')
    ...
    with open(input_file, 'r', encoding = 'utf-8') as f:
        ...
        for line in sliced:
            try:
                ...
                logger.warning(f'Skipping invalid JSON line: {0}')
                continue
...
def _parse_ollama_responses(response_text: str): 
    try:
    ...
    for idx, review in enumerate(healed_reviews):
        ...
        for attempt in range(Config.OLLAMA_RETRIES):
            try:
                ...
                prompt = f"""
                    ANAlyze the sentiment of this review and classify it as POSITIVE, NEGATIVE, NEUTRAL
                    Review: "{text}"
                ...
                prediction = _parse_ollama_responses(response_text)
                break
...
@dag(
        ...
        description='Pipeline fro sentiment analysis using OLLAMA model',
        schedule = None,
        ...
        params = {
            'input_file': Param(
                ...
                type = 'string',
                description = 'Path to the input JSON file containing Yelp review.'
                ),
            'batch_size': Param(
            ...
            'ollama_model': Param(
                ...
                type = 'string',
                description = 'Name of the OLLAMA model to use sentiment analysis.'
                ),
            },
...
def self_healing_pipeline():
    ...
    def load_reviews():
        ...
        logger.info(f'Loading reviews with batch sie {batch_size} and offset {offset}')
    ...
    def diagnose_and_heal_batch(reviews: list[dict]):
        ...
        logger.info(f'Healed {healed_count} out of {len(reviews)} reviews in batch.')
    ...
    def aggregate_results(results: list[list[dict]]):
        ...
        for r in results:
            ...
            star_sentiment[f'{int(stars)}_star'][sentiment] += 1
        ...
        summary = {
                "run_info": {
                    ...
                    'batch_size': params.get('offset', Config.DEFAULT_OFFSET),
                    'offset': params.get('offset', Config.DEFAULT_OFFSET),
                    'input_file': params.get('input_file', Config.INPUT_FILE),
                },
                'totals': {
    ...
    def generate_health_report(summary: dict):
        ...
        logger.info(f'Success Rate: {summary["rates"]["success_rate"]}, Healing Rate: {summary["rates"]["healing_rate"]}, Degradation Rate: {summary["rates"]["degradation_rate"]}')
